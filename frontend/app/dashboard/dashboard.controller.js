angular.module("dashboard").
    controller("dashboardController" , ["coreService", "dashboardService","$rootScope",
        ...
        this.gridsterOpts = {
            ...
            $rootScope.drawData = function(checkedColumns = null){
                // read document for this function
                var items = dashboardService.renderItems(allData[allData.length-1]['data']);
                if (checkedColumns && checkedColumns.length > 1) {
                    var names = checkedColumns.map(col => col.name);
                    items = items.filter(item => names.includes(item.title))
                }
